...
const DataSalaryEmployee = () => {
  ...
  const totalPages = Math.ceil(dataSalaryEmployee.length / ITEMS_PER_PAGE);
  ...
  const onSubmitPrint = async () => {
    ...
    try {
      const yearData = viewSalarySingleEmployeeByYear(dataYear);
      const monthData = viewSalarySingleEmployeeByMonth(dataMonth);
      const nameData = viewSalarySingleEmployeeByName(employeeName);

      if (yearData.length > 0 && monthData.length > 0 && nameData.length > 0) {
        navigate(`/data-salary-employee/print-page?month=${dataMonth}&year=${dataYear}&name=${employeeName}`);
      } else {
  ...
  useEffect(() => {
    const getDataEmployee = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/data_salary/name/${employeeName}`);
        const data = response.data;

        setDataSalaryEmployee(data);
        ...
        <div className="mt-4 flex flex-col items-center justify-between md:flex-row md:justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-gray-5 dark:text-gray-4 py-4 text-sm">
              ...
              {Math.min(endIndex, dataSalaryEmployee.length)} dari{" "}
              {dataSalaryEmployee.length} Data Salary
            </span>
          </div>
...
export default DataSalaryEmployee;
