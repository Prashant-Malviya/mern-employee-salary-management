...
const getEmployeeId = (employee) => employee.employeeId || employee.id_pegawai || '';
const getEmployeeName = (employee) => employee.employeeName || employee.nama_pegawai || 'Unknown Employee';
const getEmployeePosition = (employee) => employee.position || employee.jabatan || '';
const getEmployeeNationalId = (employee) => employee.nationalId || employee.nik || '';
...
const OvertimeForm = () => {
    ...
    useEffect(() => {
        const userRole = user?.role || user?.hak_akses;
        if (isError) {
            navigate('/login');
        }
                                    ...
                                    <tr key={entry.id} className="border-b border-[#eee] dark:border-strokedark">
                                        <td className="py-5 px-4">
                                            <p className="font-medium text-black dark:text-white">
                                                {entry.employee?.employeeName || entry.employee?.nama_pegawai || entry.employee_id}
                                            </p>
                                            <p className="text-sm text-gray-5 dark:text-gray-4">
                                                {entry.employee?.nationalId || entry.employee?.nik || getEmployeeNationalId(entry.employee || {})}
                                            </p>
                                        </td>
