...
import { deleteDataPosition, getDataPosition, getMe } from '../../../../config/redux/action';
...
const DataPosition = () => {
    ...
    const totalPages = Math.ceil(dataPosition.length / ITEMS_PER_PAGE);

    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;

    const filteredDataPosition = dataPosition.filter((position) => {
    ...
    const onDeletePosition = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(deleteDataPosition(id)).then(() => {
                    Swal.fire({
                        title: 'Success',
                        text: 'Data position berhasil dihapus.',
                        icon: 'success',
                        timer: 1000,
    ...
    useEffect(() => {
        dispatch(getDataPosition(startIndex, endIndex));
    ...
    return (
        <Layout>
            <Breadcrumb pageName='Data Position' />
            ...
            <div className='rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-6'>
                ...
                <div className='max-w-full overflow-x-auto py-4'>
                    <table className='w-full table-auto'>
                        ...
                        <tbody>
                            {filteredDataPosition.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    <tr key={data.id}>
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Menampilkan {startIndex + 1}-{Math.min(endIndex, filteredDataPosition.length)} dari {filteredDataPosition.length} Data Position
                        </span>
                    </div>
