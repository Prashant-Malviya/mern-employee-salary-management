...
import { deleteDataDeduction, getDataDeduction, getMe } from '../../../../config/redux/action';
...
const DataDeduction = () => {
    ...
    const { dataDeduction } = useSelector((state) => state.dataDeduction);

    const totalPages = Math.ceil(dataDeduction.length / ITEMS_PER_PAGE);

    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;

    const filteredDataDeduction = dataDeduction.filter((deductionSalary) => {
        const { deduction } = deductionSalary;
    ...
    const onDeleteDeduction = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(deleteDataDeduction(id)).then(() => {
                    Swal.fire({
                        title: 'Success',
    ...
    useEffect(() => {
        dispatch(getDataDeduction(startIndex, endIndex));
    ...
    return (
        <Layout>
            <Breadcrumb pageName='Data Deduction' />
            ...
            <div className='rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-6'>
                ...
                <div className='max-w-full overflow-x-auto py-4'>
                    <table className='w-full table-auto'>
                        ...
                        <tbody>
                            {filteredDataDeduction.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    <tr key={data.id}>
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <p className='text-black dark:text-white'>Rp. {data.deductionAmount}</p>
                                        </td>
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Menampilkan {startIndex + 1}-{Math.min(endIndex, filteredDataDeduction.length)} dari {filteredDataDeduction.length} Data Deduction
                        </span>
                    </div>
