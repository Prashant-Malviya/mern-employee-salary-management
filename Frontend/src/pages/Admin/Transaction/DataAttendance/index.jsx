...
import { deleteDataAttendance, getDataAttendance, getMe } from '../../../../config/redux/action';
...
const DataAttendance = () => {
    ...
    const filteredDataAttendance = dataAttendance.filter((attendanceData) => {
        const isMatchMonth =
            filterMonth === "" ||
            attendanceData.month.toLowerCase().includes(filterMonth.toLowerCase());
        const isMatchYear =
            filterYear === "" || attendanceData.tahun.toString() === filterYear;
        const isMatchNama =
            filterNama === "" ||
            attendanceData.employeeName.toLowerCase().includes(filterNama.toLowerCase());
        return isMatchMonth && isMatchYear && isMatchNama;
    });
    ...
    const onDeleteDataAttendance = (id) => {
        Swal.fire({
            title: 'Konfirmasi',
            ...
            if (result.isConfirmed) {
                dispatch(deleteDataAttendance(id)).then(() => {
                    Swal.fire({
                        title: 'Success',
    ...
    useEffect(() => {
        dispatch(getDataAttendance(startIndex, endIndex));
    ...
    return (
        <Layout>
            <Breadcrumb pageName='Attendance Data Employee' />
            ...
            <div className='rounded-sm border border-stroke bg-white px-5 pt-2 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-10 mt-6'>
                <div className='border-b border-stroke py-2 dark:border-strokedark'>
                    <h3 className='font-medium text-black dark:text-white'>
                        Filter Attendance Data Employee
                    </h3>
                </div>
                ...
                <div className="bg-gray-2 text-left dark:bg-meta-4 mt-6">
                    {filteredDataAttendance.slice(startIndex, endIndex).reduce((uniqueEntries, data) => {
                        ...
                        <h2 className="px-4 py-2 text-black dark:text-white" key={`${data.month}-${data.tahun}`}>
                            Menampilkan Attendance Data Employee Month :
            ...
            <div className='rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-6'>
                ...
                <div className='max-w-full overflow-x-auto py-4'>
                    <table className='w-full table-auto'>
                        ...
                        <tbody>
                            {filteredDataAttendance.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    <tr
                                        key={data.id}
                                    ...
                                    >
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <p className='text-black text-center dark:text-white'>{data.nationalId}</p>
                                        </td>
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <div className='flex items-center space-x-3.5'>
                                                ...
                                                <button className='hover:text-black'
                                                    onClick={() => onDeleteDataAttendance(data.id)}
                                                >
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Menampilkan {startIndex + 1}-{Math.min(endIndex, filteredDataAttendance.length)} dari {filteredDataAttendance.length} Attendance Data Employee
                        </span>
                    </div>
